...
class AccountForm(LoginRequiredMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        fields = '__all__'
        widgets = {
            'name': forms.TextInput(attrs={'class': 'form-control', 'id': 'name', 'placeholder': 'ex. Chase Bank'})
        }
...
class SubCategoryForm(LoginRequiredMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'name': forms.TextInput(attrs={'class': 'form-control', 'id': 'name'}),
            'amount': forms.TextInput(attrs={'class': 'form-control', 'id': 'amount'}),
            'type': forms.RadioSelect(attrs={'class': 'form-check-input', 'id': 'type'}),
            'fund': forms.RadioSelect(attrs={'class': 'form-check-input', 'id': 'fund'})
        }

    def clean(self):
        cleaned_data = super().clean()

        # This prevents a user from creating a savings subcategory that is not a Sinking Fund
...
class TransactionForm(LoginRequiredMixin, forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'expense': forms.Select(attrs={'class': 'form-control', 'id': 'expense'}),
            ...
            'vendor': forms.TextInput(attrs={'class': 'form-control', 'id': 'vendor'}),
            'account': forms.Select(attrs={'class': 'form-control', 'id': 'account'}),
            'notes': forms.Textarea(attrs={'class': 'form-control', 'id': 'notes', 'rows': '5'}),
        }

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        #######################################
        # This code limits the choices of the subcategory field to those created by the currently logged in user.
        #    'initial' data is passed to the form by the TransactionCreateView class in views.py.
        ...
        self.fields['subcategory'].choices = (
            (subcategory.id, f"{Category.objects.get(id=subcategory.category_id)} - {subcategory}") for subcategory in subcategories)
        self.fields['account'].choices = [
            (account.id, account.name) for account in Account.objects.filter(user=kwargs['initial']['user'])
        ]
